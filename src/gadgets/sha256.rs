...
use crate::{fields::Field, ConstraintSystem, SynthesisError};
    ...
    fn test_blank_hash() {
        ...
        impl Circuit<Fp> for TestCircuit {
            fn synthesize<CS: ConstraintSystem<Fp>>(
                ...
                let out = sha256_compression_function(cs, &input_bits, &iv).unwrap();
        ...
        impl Circuit<Fp> for TestCircuit {
            fn synthesize<CS: ConstraintSystem<Fp>>(
                ...
                let mut rng = XorShiftRng::from_seed([
                    0x59, 0x62, 0xbe, 0x3d, 0x76, 0x3d, 0x31, 0x8d, 0x17, 0xdb, 0x37, 0x32, 0x54,
                    0x06, 0xbc, 0xe5,
                ]);
                ...
                sha256_compression_function(cs.namespace(|| "sha256"), &input_bits, &iv)?;
    ...
    fn test_against_vectors() {
        ...
        impl Circuit<Fp> for TestCircuit {
            fn synthesize<CS: ConstraintSystem<Fp>>(
                ...
                for (byte_i, input_byte) in self.data.iter().enumerate() {
        ...
        for input_len in (0..32).chain((32..256).filter(|a| a % 8 == 0)) {
            ...
            assert_eq!(
                is_satisfied::<_, _, Basic>(&TestCircuit::new(data), &[]),
                Ok(true)
            );
        }
    }
