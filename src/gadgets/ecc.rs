...
impl<C: Curve> CurvePoint<C> {
    ...
    pub fn add_conditionally<CS: ConstraintSystem<C::Base>>(
        ...
        let p1 = self
            .x
            .value()
            .and_then(|x| self.y.value().and_then(|y| Some(C::from_xy(x, y).unwrap())));

        let p2 = other.x.value().and_then(|x| {
            other
                .y
                .value()
                .and_then(|y| Some(C::from_xy(x, y).unwrap()))
        });
        ...
        let x3 = AllocatedNum::alloc(cs, || Ok(p3.ok_or(SynthesisError::AssignmentMissing)?.0))?;

        let y3 = AllocatedNum::alloc(cs, || Ok(p3.ok_or(SynthesisError::AssignmentMissing)?.1))?;
